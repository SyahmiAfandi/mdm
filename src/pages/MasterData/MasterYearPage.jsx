...
function makeIdFromYear(year) {
  // stable doc id so you don’t get duplicates
  return `Y${normalize(year).replace(/\s+/g, "")}`;
}
...
export default function MasterYearPage() {
  ...
  const [sortKey, setSortKey] = useState("year"); // year | name | activeLabel
  ...
  const filtered = useMemo(() => {
    ...
    let out = rows.map((r) => ({
      ...
      yearText: (r.year ?? "").toString(),
    }));

    if (t) {
      out = out.filter((r) => {
        const hay = `${r.yearText} ${r.name || ""} ${r.activeLabel || ""}`.toLowerCase();
  ...
  function openEdit(row) {
    ...
    setForm({
      ...
      name: row.name || (row.year != null ? String(row.year) : ""),
  ...
  async function onSave() {
    ...
    const yearRaw = normalize(form.year);
    const name = normalize(form.name) || yearRaw;
    const active = form.status !== "Inactive";

    if (!yearRaw) return toast.error("Year is required");
    if (!isValidYear(yearRaw)) return toast.error("Year must be between 1900 and 2100");
    ...
    try {
      ...
      if (mode === "create") {
        const id = makeIdFromYear(yearRaw);
        await setDoc(
          doc(db, COL, id),
          {
            year: yearNum,
        ...
        await updateDoc(doc(db, COL, editingId), {
          // year locked by design (doc id = year)
          name,
          active,
          updatedAt: serverTimestamp(),
          updatedBy: CURRENT_USER,
  ...
  async function onDelete(row) {
    ...
    const ok = confirm(`Delete year "${row.year}"?`);
    if (!ok) return;
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                You don’t have permission to view Years master.
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-start gap-3">
          <div className="h-10 w-10 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
            <CalendarDays className="w-5 h-5 text-gray-900 dark:text-gray-100" />
          </div>
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Maintain years for dropdowns and reporting filters.
            </p>
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search year / name / status…"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 dark:bg-gray-800 dark:text-gray-200">
              <tr>
                <th className="px-4 py-3 text-left font-semibold w-[140px]">Year</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {filtered.map((r) => (
                <tr key={r.id} className="text-gray-800 dark:text-gray-100">
                  ...
                  <td className="px-4 py-3">{r.name || r.year}</td>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl dark:bg-gray-900">
            ...
            <div className="px-5 py-4 space-y-3">
              <div>
                ...
                {mode === "edit" && (
                  <div className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                    Year is locked in edit mode (doc id is based on year).
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                />
                <div className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                  Optional (if empty, we’ll use the year).
                </div>
              </div>
